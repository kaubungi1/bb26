...
function jacket(s){const parts=ordered(s),me=Nick.get();
 const shot=s.hasThumb
  ? `<img class="jk-img" src="/api/songs/${s.id}/thumb" alt="" loading="lazy" decoding="async">`
...
function slot(p,me){const mine=p.supports.find(x=>x.nickname===me);
 /* 내 이름을 맨 앞으로. 칸이 좁아 잘려도 내가 먼저 보인다. 나머지는 지원 순서 그대로. */
 const names=[...(mine?[mine]:[]),...p.supports.filter(x=>x!==mine)].map(supportName);const full=names.join(', ');
...
function preloadThumbs(id,span=4){const a=list();const i=a.findIndex(s=>s.id===id);if(i<0)return;
 for(let d=-span;d<=span;d++){const s=a[(i+d+a.length)%a.length];
  ...
  warmed.add(s.id);const im=new Image();im.decoding='async';im.src=`/api/songs/${s.id}/thumb`}}
...
function skeleton(){const bar=n=>`<span class="sk-bar" style="width:${n}%"></span>`;return `<div class="selection-layout"><section class="selector">${controls()}<div class="carousel is-skeleton">${[0,1,2].map(i=>`<div class="track${i===0?' chosen':''}"><div class="jacket sk genre-other"><div class="jk-shot"></div><div class="jk-band"><b>&nbsp;</b><small>&nbsp;</small></div><div class="slots">${[0,1,2,3,4,5].map(()=>`<i class="slot"><em>&nbsp;</em><u>&nbsp;</u></i>`).join('')}</div></div></div>`).join('')}</div><div class="transport"><button type="button" disabled>◀</button><span id="position">···</span><button type="button" disabled>▶</button><button type="button" disabled>▷ 자동</button></div></section><aside class="my-rail"><span class="overline"><span>MY ACTIVITY</span></span><h2>내 일정 · 참여</h2><p class="sk-line">${bar(70)}</p><a class="sk-row" aria-hidden="true">지원한 곡<b>${bar(30)}</b></a><button type="button" disabled>내 프로필 편집</button></aside></div><section class="meetings"><div class="section-heading"><h2>모임 · 일정</h2><a href="${Site.base}/schedule/">전체 보기 ↗</a></div><div class="event-grid">${[0,1].map(()=>`<div class="meeting sk-meeting"><div><small>${bar(28)}</small><h3>${bar(62)}</h3><p>${bar(44)}</p></div></div>`).join('')}</div></section>`}
...
function controls(){const active=prefs.genres.length+prefs.guilds.length+(!!prefs.part);const genres=genreList;return `<div class="selector-head"><div><span class="overline"><span>MUSIC SELECT</span></span><h1>곡 선택<small>${songs.length?list().length:''}</small></h1></div><a class="suggest-button" href="${Site.base}/songs/?add=1">＋ 곡 등록</a></div><div class="tools"><label class="search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg><input id="search" type="search" placeholder="곡명 · 아티스트 · 사람" aria-label="곡명 · 아티스트 · 사람 검색" value="${esc(prefs.query)}"></label><label class="sortbox"><select id="sort" aria-label="정렬">${[['recommend','추천'],['random','무작위'],['new','최근 등록'],['name','곡명순']].map(([v,t])=>`<option value="${v}" ${prefs.sort===v?'selected':''}>${t}</option>`).join('')}</select></label><button type="button" id="shuffle" class="icon-btn" title="다시 섞기" aria-label="다시 섞기">↻</button></div><div class="chips" role="group" aria-label="필터" ${chipsOpen?'':'data-collapsed'}><button type="button" class="chip-toggle" data-chips aria-expanded="${chipsOpen}">필터${active?` <b>${active}</b>`:''}</button>${genres.map(g=>`<button type="button" class="chip" data-filter="genres" value="${esc(g)}" aria-pressed="${prefs.genres.includes(g)}">${esc(shortGenre(g))}</button>`).join('')}<span class="chip-gap"></span><span class="chip-label">빈 자리</span>${ORDER.filter(r=>parts.includes(r)).map(r=>`<button type="button" class="chip part" data-part="${esc(r)}" aria-pressed="${prefs.part===r}">${esc(abbr(r))}</button>`).join('')}${guildList.length?`<label class="sortbox guildbox"><select id="guild" aria-label="길드"><option value="">전체</option><option value="none" ${prefs.guilds[0]==='none'?'selected':''}>${SITE_NAME}</option>${guildList.map(g=>`<option value="${esc(g.slug)}" ${prefs.guilds[0]===g.slug?'selected':''}>${esc(g.name)}</option>`).join('')}</select></label>`:''}${active?'<button type="button" id="reset" class="chip clear">필터 해제 ×</button>':''}</div>`;}
/* 한 칸. side 가 'L'/'R' 이면 양 끝에 덧붙인 복제본이다 — 읽히지도, 탭으로 닿지도 않는다. */
function trackWrap(s,side){const c=side?` data-clone="${side}"`:'';return `<div class="track-wrap" data-track="${s.id}"${c}${side?' aria-hidden="true"':''}><button class="track ${s.id===selected?'chosen':''}" data-track="${s.id}"${side?' tabindex="-1"':''} aria-label="${esc(s.title)}" aria-pressed="${s.id===selected}">${jacket(s)}</button>${s.youtubeUrl?`<a class="yt" href="${esc(s.youtubeUrl)}"${side?' tabindex="-1"':''} target="_blank" rel="noreferrer noopener" aria-label="${esc(s.title)} 유튜브에서 보기" title="유튜브에서 보기"><svg viewBox="0 0 24 24" aria-hidden="true"><path class="body" d="M21.6 7.2a2.6 2.6 0 0 0-1.8-1.8C18.2 5 12 5 12 5s-6.2 0-7.8.4A2.6 2.6 0 0 0 2.4 7.2 27 27 0 0 0 2 12a27 27 0 0 0 .4 4.8 2.6 2.6 0 0 0 1.8 1.8C5.8 19 12 19 12 19s6.2 0 7.8-.4a2.6 2.6 0 0 0 1.8-1.8A27 27 0 0 0 22 12a27 27 0 0 0-.4-4.8z"/><path class="play" d="M10 15.2V8.8L15.5 12z"/></svg></a>`:''}</div>`}
/* 끝과 처음을 이어 보이게 앞뒤로 세 장씩 덧붙인다. 세 장 이하 목록은 이을 폭이 없어 그냥 둔다. */
const LOOP_PAD=3;
function loop(a){if(a.length<=LOOP_PAD)return a.map(s=>trackWrap(s,'')).join('');
 return [...a.slice(-LOOP_PAD).map(s=>trackWrap(s,'L')),...a.map(s=>trackWrap(s,'')),...a.slice(0,LOOP_PAD).map(s=>trackWrap(s,'R'))].join('')}
function results(){cleanupCarousel();const a=list();const count=$('.selector-head h1 small');if(count)count.textContent=a.length;if(!a.some(s=>s.id===selected))selected=a[0]?.id;$('#results').innerHTML=a.length?`<div class="carousel" tabindex="0" aria-label="곡 선택. 좌우 방향키로 이동">${loop(a)}</div><div class="transport"><button id="prev" aria-label="이전 곡">◀</button><span id="position"></span><button id="next" aria-label="다음 곡">▶</button><button id="auto" aria-pressed="${auto}">${auto?'Ⅱ 정지':'▷ 자동'}</button><a id="song-link" href="${Site.base}/songs/">곡 정보 ↗</a><a href="${Site.base}/songs/">목록 ↗</a></div><div id="selected-detail"></div>`:ghost();if(a.length){choose(selected,false);bindCarousel();}persist();}
function choose(id,smooth=true){if(!$('#selected-detail')||!songs.some(s=>s.id===id)||!list().some(s=>s.id===id))return;selected=id;const s=songs.find(s=>s.id===id);document.querySelectorAll('.track').forEach(b=>{b.classList.toggle('chosen',+b.dataset.track===id);b.setAttribute('aria-pressed',+b.dataset.track===id)});const el=$(`[data-track="${id}"]:not([data-clone])`);center(el,smooth);tickOpen=false;tickIdx=0;clearInterval(ticker);ticker=null;$('#selected-detail').innerHTML=detail(s);$('#position').textContent=`${list().findIndex(s=>s.id===id)+1} / ${list().length}`;const sl=$('#song-link');if(sl)sl.href=`${Site.base}/songs/?song=${id}`;preloadThumbs(id);loadComments(id);persist();}
...
/* 목록 양 끝에 붙인 복제본 위에 멈췄으면 같은 내용의 진짜 칸으로 순간이동한다.
   그림이 같아서 눈에 안 띈다. 이게 '끝과 처음이 이어져 보이는' 장치의 전부다. */
function unclone(el){if(!el||!el.hasAttribute('data-clone'))return false;choose(+el.dataset.track,false);return true}
function stop(){auto=false;clearInterval(timer);if($('#auto')){$('#auto').textContent='▷ 자동';$('#auto').setAttribute('aria-pressed','false')}}
function step(n){const a=list(),i=a.findIndex(s=>s.id===selected),j=i+n;
 if(j>=0&&j<a.length)return choose(a[j].id);
 /* 끝을 넘어갈 때. 반대쪽 끝까지 쭉 훑지 않고, 바로 옆에 붙여 둔 복제본으로 넘어간 뒤
    멈추는 순간 진짜 칸으로 옮겨 앉는다(scrollend → unclone). */
 const c=$('.carousel'),id=a[(j+a.length)%a.length].id;
 if(!c)return choose(id);
 const cl=[...c.querySelectorAll(`[data-clone="${j<0?'L':'R'}"]`)];
 const el=j<0?cl[cl.length-1]:cl[0];
 if(!el||+el.dataset.track!==id)return choose(id);
 center(el,true);
 /* scrollend 를 지원 안 하는 브라우저를 위한 보험. 늦게라도 진짜 칸에 앉힌다. */
 clearTimeout(loopFix);loopFix=setTimeout(()=>choose(id,false),700);}
let loopFix=null;
function bindCarousel(){const c=$('.carousel');let x,start,last,t,velocity=0,moved=false,raf,settle;cleanupCarousel=()=>{cancelAnimationFrame(raf);clearTimeout(settle);drag=false;c.onpointerdown=null;c.onpointermove=null;c.onpointerup=null;c.onpointercancel=null;c.onwheel=null;};function nearest(){const cr=c.getBoundingClientRect(),center=cr.left+cr.width/2,dist=el=>{const r=el.getBoundingClientRect();return Math.abs(r.left+r.width/2-center)};return [...c.children].reduce((a,b)=>dist(b)<dist(a)?b:a)}c.onpointerdown=e=>{if(e.button!==0)return;stop();cancelAnimationFrame(raf);drag=true;moved=false;x=e.clientX;start=c.scrollLeft;last=x;t=performance.now();velocity=0;c.style.scrollSnapType='none'};c.onpointermove=e=>{if(!drag)return;const now=performance.now();if(Math.abs(e.clientX-x)>5){moved=true;c.setPointerCapture(e.pointerId)}velocity=(last-e.clientX)/Math.max(1,now-t);last=e.clientX;t=now;c.scrollLeft=start+x-e.clientX};function finish(){if(!drag)return;drag=false;if(!moved){c.style.scrollSnapType='x mandatory';return}let v=Math.max(-45,Math.min(45,velocity*16));function glide(){c.scrollLeft+=v;v*=.93;if(!reducedMotion&&Math.abs(v)>.6)raf=requestAnimationFrame(glide);else{c.style.scrollSnapType='x mandatory';const el=nearest();if(!unclone(el))choose(+el.dataset.track)}}raf=requestAnimationFrame(glide)}c.onpointerup=finish;c.onpointercancel=()=>{drag=false;cancelAnimationFrame(raf);c.style.scrollSnapType='x mandatory';const el=nearest();if(!unclone(el))choose(+el.dataset.track)};c.addEventListener('click',e=>{if(moved){e.preventDefault();e.stopPropagation();moved=false}},true);;c.addEventListener('scrollend',()=>{if(c.isConnected&&!drag){const el=nearest();clearTimeout(loopFix);if(unclone(el))return;const id=+el.dataset.track;if(id!==selected)choose(id,false);}});c.onwheel=()=>{stop();clearTimeout(settle);settle=setTimeout(()=>{const el=nearest();if(!unclone(el))choose(+el.dataset.track)},180)};$('#prev').onclick=()=>{stop();step(-1)};$('#next').onclick=()=>{stop();step(1)};$('#auto').onclick=()=>{if(auto)return stop();auto=true;$('#auto').textContent='Ⅱ 정지';$('#auto').setAttribute('aria-pressed','true');timer=setInterval(()=>{if(!document.hidden&&!c.matches(':hover')&&!c.contains(document.activeElement))step(1)},2000)};}
...
document.addEventListener('keydown',e=>{
 ...
 e.preventDefault();stop();step(e.key==='ArrowLeft'?-1:1);
});
